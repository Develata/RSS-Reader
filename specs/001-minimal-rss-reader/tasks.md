...
续阅读体验增强路线，单独在设计文档中约束，不在当前任务清单里伪造为已完成。前端命令
面与公开界面接口另行记录在 `docs/design/frontend-command-reference.md`，用于约束后续
主题、自定义 CSS 与 CLI/UI 一致性演进。配置交换中保留的 `folder` 字段仅用于 OPML /
配置包互操作保真，不构成当前产品主线中的文件夹能力。
...
并确保产品范围持续收敛在订阅、阅读、基本设置和基础配置交换之内。
...
- [X] T010 [P] 预留 Web 持久化适配层入口，并在当前实现中使用浏览器本地持久化状态 `crates/rssr-infra/src/db/sqlite_web.rs`
...
**目的**：对照“功能设计哲学”清理无效设置项与部署包装层歧义，并补齐订阅能力中已明确定义但尚未完全落地的行为。

- [X] T061 对齐自动刷新：让 `refresh_interval_minutes` 真正驱动启动后 / 后台运行时的自动订阅拉取 `crates/rssr-app/src/bootstrap.rs`
- [X] T062 对齐订阅状态展示：扩展 `FeedSummary` 与订阅页，展示 URL、本地文章数量、最近刷新状态和失败反馈 `crates/rssr-domain/src/feed.rs`
- [X] T063 清理设置占位：让 `list_density`、`startup_view`、`reader_font_scale` 真正影响界面 / 启动路径，或缩减为当前已真实生效的设置集合 `crates/rssr-app/src/pages/settings_page.rs`
- [X] T064 收紧 Web 包装层边界：把 `rssr-app` 内的本地登录门禁明确限制为开发 / 浏览器本地数据保护用途，并避免与 `rssr-web` 服务端登录形成功能重复 `crates/rssr-app/src/web_auth.rs`
- [X] T065 对齐阅读组织：在文章页支持按来源与按时间两种分类 / 分组浏览方式，并保持直接进入阅读的低摩擦流 `crates/rssr-app/src/pages/entries_page.rs`
- [X] T066 对齐自动归档：为文章增加归档状态，并让默认 3 个月、可配置阈值的自动归档真实驱动文章列表展示 `crates/rssr-domain/src/entry.rs`
- [X] T067 收敛阅读风格借鉴边界：在不引入文档站壳层的前提下，把阅读排版和列表层级继续向克制、清晰、文档化阅读体验靠拢 `assets/styles.css`
- [X] T068 扩展文章页筛选模型：支持仅已读、仅未收藏与来源多选筛选，并同步 native / web 查询实现 `crates/rssr-domain/src/entry.rs`
- [X] T069 增强按时间阅读目录：在文章页提供“月份 → 来源 → 日期”的轻量目录层级，并保持移动端可用 `crates/rssr-app/src/pages/entries_page.rs`
...
1. 开发者 A：应用服务与领域模型。
2. 开发者 B：基础设施、抓取与存储适配。
3. 开发者 C：Dioxus 页面、组件与交互。
4. 在 Phase 2 完成后，按用户故事拆分并行推进，实现后再统一回归验证。
