...
- [ ] T065 对齐阅读组织：在文章页支持按来源与按时间两种分类 / 分组浏览方式，并保持直接进入阅读的低摩擦流 `crates/rssr-app/src/pages/entries_page.rs`
- [ ] T066 对齐自动归档：为文章增加归档状态，并让默认 3 个月、可配置阈值的自动归档真实驱动文章列表展示 `crates/rssr-domain/src/entry.rs`
