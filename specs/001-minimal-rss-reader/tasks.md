...
- [X] T010 [P] 在当前实现中使用浏览器本地持久化状态，并避免保留误导性的 Web SQLite 占位入口
...
- [X] T069 增强文章阅读目录：在文章页提供“按时间时月份 → 日期 → 来源 → 文章、按来源时来源 → 月份”的轻量目录层级，并保持移动端可用 `crates/rssr-app/src/pages/entries_page.rs`
