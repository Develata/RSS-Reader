...
pub(crate) fn AppearanceSettingsCard(facade: SettingsPageFacade) -> Element {
    rsx! {
        div { class: "settings-card", "data-layout": "settings-card", "data-section": "appearance",
            div { class: "settings-card__header", "data-slot": "settings-card-header",
                h3 { class: "card-title", "data-slot": "card-title", "阅读外观" }
            }
            ReadingPreferencesSection { facade: facade.clone() }
            ThemeSettingsSections { facade: facade.clone() }
            div { class: "settings-card__footer", "data-layout": "settings-card-footer",
                button {
                    class: "button",
