...
use crate::datetime::format_datetime_utc;
...
pub(super) fn feed_refresh_status_text(feed: &FeedSummary) -> String {
    match (feed.last_success_at, feed.last_fetched_at) {
        (Some(last_success_at), Some(last_fetched_at)) if last_fetched_at > last_success_at => {
            format!(
                "最近尝试：{} · 最近成功：{}",
                format_datetime_utc(Some(last_fetched_at)).unwrap_or_else(|| "未知".to_string()),
                format_datetime_utc(Some(last_success_at)).unwrap_or_else(|| "未知".to_string())
            )
        }
