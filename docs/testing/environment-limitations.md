...
- `env-limited` 结论必须同时记录：
  ...
  - 失败入口
  - 复验方式
- 如果后续发现某限制已经不再成立，应更新本页，而不是只在新的 handoff 中零散备注
...
- 受影响入口：
  - [test_webdav_local_roundtrip.rs](/Users/vegetable/Desktop/RSS-Reader/crates/rssr-infra/tests/test_webdav_local_roundtrip.rs)
...
- 为什么不算代码回归：
  - 已有记录表明在允许本地端口绑定的环境中，该测试可通过
  - 2026-04-08 的全量验证记录已确认问题来自环境权限，而不是配置交换逻辑本身
- 规避方式 / 复验方式：
  - 在受限环境下，将该项明确标记为 `env-limited`
  - 在允许 loopback 端口绑定的环境中单独复跑 `cargo test -p rssr-infra --test test_webdav_local_roundtrip`
  - 如果复跑通过，则本轮不应记为主线功能回归
- 参考记录：
  - [../handoffs/2026-04-08-full-test-verification.md](../handoffs/2026-04-08-full-test-verification.md)
  - [../handoffs/2026-04-08-hotfix-html-paste-web-startup.md](../handoffs/2026-04-08-hotfix-html-paste-web-startup.md)
...
- 为什么不算代码回归：
  ...
  - 已有浏览器回归记录表明，同样的 feed 在 `rssr-web` 同源代理模式下可成功导入或刷新
- 规避方式 / 复验方式：
  - Web smoke 优先使用已知开放 CORS 的 feed 源
  - 需要验证跨源抓取时，优先通过 `rssr-web` 的同源代理模式复验
  - 记录失败时，必须注明是“纯静态 Web 直连”还是“`rssr-web` 代理”模式
- 参考记录：
  - [global-browser-regression.md](./global-browser-regression.md)
...
- 为什么不算代码回归：
  - 已有桌面最终验收记录显示主功能链路可通过
  - 问题更像宿主图形环境或窗口管理行为，不是阅读、订阅、刷新、配置交换等主 use case 的业务回归
- 规避方式 / 复验方式：
  - 记录为宿主环境备注，不单独阻塞主线功能验证
  - 如需确认是否为应用回归，应在非 WSLg 桌面环境复验相同步骤
- 参考记录：
  - [manual/final-acceptance-checklist.md](./manual/final-acceptance-checklist.md)

## 记录模板

后续新增环境限制时，建议统一使用下面的结构：
