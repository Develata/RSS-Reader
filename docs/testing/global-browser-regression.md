# 全局浏览器回归报告

本报告记录最近一轮基于 Chrome MCP 的 Web 端全链路人工回归结果。
测试目标是确认当前浏览器可访问功能在真实页面交互下可正常使用，并把通过项沉淀为可追溯记录。

## 测试环境

- 时间：2026-04-02
- 运行方式：
  - `dx bundle --platform web --package rssr-app --release --out-dir target/web-e2e`
  - 静态 bundle 通过本地 SPA fallback 服务访问 `http://127.0.0.1:8054`
  - `rssr-web` 通过本地服务访问 `http://127.0.0.1:8060`
- 浏览器：Chrome MCP
- 主要远端 feed：
  - `https://github.blog/feed/`
  - `https://blogs.nvidia.com/feed/`
  - `https://www.ruanyifeng.com/blog/atom.xml`
- WebDAV 测试端点：`http://127.0.0.1:8052/config.json`
- 移动端视口：`390 x 844`

## 结论

当前浏览器可测试的核心功能均已通过。
本轮额外修复了两个浏览器侧问题后重新复测通过：

- wasm 端运行时不再因为 `tracing_subscriber` 的时间格式化而 panic
- 本地 Web 登录页不再出现“密码框不在 form 内”的浏览器告警
- `rssr-web` 部署态下，CORS 受限的 Atom/RSS 源可通过同源 `/feed-proxy` 成功导入
- 小窗口下的导航、订阅页、文章页、阅读页和设置页都保持可用，无关键布局破坏
- 文章页新增的阅读筛选与时间目录层级已经在真实数据下通过复测

## 通过项

- [x] 首次打开 `/` 会进入本地 Web 登录初始化页。
- [x] 初始化本地 Web 登录后，可正常进入文章页。
- [x] 后续重新打开页面会进入登录页，输入已设置的用户名/密码后可正常进入应用。
- [x] `订阅 / 文章 / 设置` 路由切换正常。
- [x] 顶栏搜索框可直接驱动文章页标题筛选，并与文章页内搜索框保持同步。
- [x] 远端 feed `https://github.blog/feed/` 可添加并完成首次刷新。
- [x] 远端 feed `https://blogs.nvidia.com/feed/` 在浏览器直连模式下可添加并完成首次刷新。
- [x] 通过 `rssr-web` 访问时，`https://www.ruanyifeng.com/blog/atom.xml` 可成功导入并完成首次刷新。
- [x] 订阅页 `刷新全部` 可正常执行并更新状态。
- [x] 订阅页两步确认删除订阅可正常工作。
- [x] `导出配置` 可生成 JSON 配置包。
- [x] `导出 OPML` 可生成 OPML 文本。
- [x] `导入配置` 可恢复订阅与设置。
- [x] `导入 OPML` 可恢复订阅列表。
- [x] 文章页可在 `按时间 / 按来源` 两种组织方式之间切换。
- [x] 文章页支持 `仅未读 / 仅已读 / 仅收藏 / 仅未收藏` 四种阅读状态筛选。
- [x] 文章页支持按来源多选筛选，单选来源与多选来源都可正常收窄结果。
- [x] 文章页 `标已读 / 标未读` 可正常切换状态。
- [x] 文章页按时间组织时，主内容区会按 `月份 → 来源 → 日期` 展示文章。
- [x] 桌面端右侧目录在按时间组织时支持按 `月份 → 来源 → 日期` 展开导航。
- [x] 阅读页正文可正常加载并显示 HTML 内容。
- [x] 阅读页 `上一篇未读 / 下一篇未读` 可正常跳转并刷新内容。
- [x] 阅读页 `上一篇同订阅文章 / 下一篇同订阅文章` 可正常跳转并刷新内容。
- [x] 阅读页底部固定操作栏可正常执行上一篇、下一篇、标记已读、收藏。
- [x] 设置页预置主题按钮可立即生效。
- [x] 设置页主题卡片 `使用这套主题 / 移除这套主题` 可正常工作。
- [x] 设置页手动输入自定义 CSS 后，`应用当前 CSS` 可立即生效。
- [x] 设置页对无效 CSS 会给出校验错误，不会触发页面崩溃。
- [x] 设置页刷新后，自定义 CSS 与主题状态持久化正常。
- [x] 设置页 GitHub 仓库按钮可打开项目仓库页面。
- [x] WebDAV `上传配置` 可正常写入远端。
- [x] WebDAV `下载配置` 可正常恢复远端保存的设置。
- [x] 设置页表单字段具备关联标签，浏览器 Console 不再出现缺失 label / id / name 的 issue。
- [x] 本轮复测结束时，浏览器 Console 仅保留正常业务 `INFO` 日志，没有新的 panic、`unreachable` 或表单结构告警。
- [x] 在 `390 x 844` 小窗口视口下，顶部导航、订阅页、文章页、阅读页、设置页均可正常交互。
- [x] 在 `390 x 844` 小窗口视口下，订阅页的新增订阅/配置交换/已保存订阅层级保持清晰，不会退化成不可读的长表单堆叠。
- [x] 在 `390 x 844` 小窗口视口下，文章页新增筛选区和移动端目录按钮保持可用。

## 说明

- 本轮结论仅覆盖浏览器可直接验证的 Web 功能。
- 额外确认过 Docker 部署态登录门禁：
  - [x] 未登录访问 `/` 会跳转到登录页
  - [x] 错误密码会给出明确提示
  - [x] 正确密码可进入应用
  - [x] 登录后 SPA 路由仍可正常访问
  - [x] `/logout` 可清除会话并回到登录页
- Desktop、CLI、Android 不在本报告范围内。
- 纯静态 Web (`dx serve` / 仅托管 bundle) 仍受浏览器 CORS 约束；像阮一峰这类源需要通过 `rssr-web` 的同源代理访问。
- 纯静态 Web 下，像 `https://blogs.nvidia.com/feed/` 这类开放 CORS 的 feed 通常可直接使用；是否成功仍取决于目标站点的响应头策略。
- Web 端正文图片仍受浏览器 CORS 约束；这属于平台边界，不影响本轮通过结论。
