...
- **C15 主题管理三套动词/路径**（06 类，中优先级，本轮未动）：下拉「载入所选主题」/ 快捷按钮裸主题名 / 画廊卡「使用这套主题·移除这套主题」/ 实验室「应用当前 CSS」四种动词并存。归并涉及入口结构调整，按架构护栏需先提案：保守方案是只统一动词文案（全部收敛为「应用 XXX」），不动三套入口；激进方案是撤掉快捷按钮行（与画廊完全冗余）。等用户拍板。
- **C1 阅读页返回按钮位置**（01 类，低）：现为标题下方全宽按钮（absTop 191），非顶部左上惯例位；Android 有 `use_mobile_back_navigation` 兜底，是否调整属设计决策。
- **C5 目录 chips 横滑无溢出指示**：本轮只有 2 个月分组未溢出；多月分组溢出时无渐进阴影/滚动提示，低优先级。
- **来源筛选 chip 极端订阅名 180.7px 高**（正常 40.7px）：容器承载住了内容（04 类合规），视觉冲击大但不截断身份，记录为观察项；若要 line-clamp 需权衡身份可见性。
- **C7 Android 主题文件导出未接系统保存器**（`theme_file_io.rs:45` bail!）：平台能力缺口，非本轮范围。
- **C20 safe-area 观察项**：仍只覆盖 reader-bottom-bar，targetSdk 升 35 前需全量过一遍 sticky/固定元素。
- 上批遗留建议「360×800 断言固化进 smoke 脚本」仍未做；本轮的 localStorage 种子注入方法可复用为该脚本的 fixtures。
