...
按任务 2–5 已确认规格实现任务 2：列表返回锚定及文章内位置记忆，仅本次运行内生效，不自动标已读。主要 Web 专项通过；原生及 Android 环境阻塞，未达到提交条件。
...
Web 主要行为已验证；原生全量与 Android 编译仍阻塞，commit: pending。没有提交、push、tag 或 release。
