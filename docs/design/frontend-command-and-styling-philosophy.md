...
- 设置页可一键载入内置示例主题和清空当前自定义 CSS
- 设置页内置主题卡片和“载入所选主题”会立即应用并自动保存当前预置主题
- 设置页允许按主题卡片单独移除当前正在使用的内置主题，而不是只能全局清空
- 设置页可从 `.css` 文件载入主题，并把当前草稿导出为 `.css` 文件
- 设置页会显示当前样式来源，并提供预置主题下拉选择
- 设置页内置官方主题库卡片，可直接查看主题说明、主色和一键应用
...
- 正文缓存边界保持在 feed 已提供的 `content_html` / `content_text`
- 不默认抓取站点原网页做全文抽取
- 桌面端和 Android 端已经开始把正文中的部分静态图片资源本地化进缓存 HTML
- 这一步现在以“刷新成功后的后台补偿任务”执行，而不是阻塞订阅刷新主流程
- Web 端遵循同样的正文缓存边界，但图片资源仍然受浏览器 CORS 限制：
  - feed 本身可抓取，不代表正文图片也允许跨域下载
  - 因此 Web 端可能出现“正文已缓存，但图片仍保留远端 URL”的情况
  - 这属于浏览器平台约束，不是样式层或 UI 交互层问题
...
- `feed` 正文里提供多少内容，就缓存多少内容
- 不额外抓取站点原网页来做正文抽取
- 不把原网页的导航、评论、广告、脚本和站点壳层当成缓存目标

这意味着：

- `content_html`、`content_text`、`summary` 这类来自 feed 的正文数据应被本地持久化
- 如果 feed 已经提供全文 HTML，阅读器直接缓存这份全文
- 如果 feed 只提供摘要，阅读器缓存的也仅是摘要，而不是试图从原网页反向补全文

这条边界同时适用于桌面、Web 和 Android，避免不同平台走出不同的正文来源策略。

## 静态资源本地化

为了改善阅读页图片卡顿，并提升离线可读性，正文里引用的静态资源应尽可能一起本地化。
...
1. 保存 feed 正文 HTML / 文本
2. 解析正文中出现的图片地址
3. 下载图片到本地缓存目录
4. 将正文 HTML 里的远端图片地址改写为本地地址
5. 阅读页优先渲染改写后的本地 HTML

这样做的收益：

- 已经同步过的文章图片可离线阅读
- 远端图片后续被删除时，已缓存文章仍可正常显示
- 阅读页首次打开时的图片抖动和卡顿可被显著减轻

当前仓库已经明确采用这条策略作为后续实现方向，但“正文静态资源本地化”本身仍应被视为
后续阅读体验增强工作，而不是已默认完成的能力。

当前实现进度进一步明确为：

- 桌面端 / Android：优先尝试把正文中的远端图片转成可离线读取的本地化内容
- 为了保证刷新稳定性，这项处理应放在刷新成功后的后台补偿路径中，而不是让用户点击刷新时同步等待
- Web：继续缓存 feed 提供的正文，但图片本地化只在目标资源允许跨域访问时才可能成立
- 因此“跨平台一致的正文缓存边界”已经成立，但“跨平台一致的图片本地化结果”目前并不成立
...
如果后续要增加“全文抓取模式”，它应被定义为单独能力，而不是正文缓存策略的默认组成部分。
...
- CSS 只控制阅读页的排版表现
- UI 不直接决定缓存规则
- 本地优先语义保持清晰：阅读器缓存的是 feed 正文及其静态阅读资源，而不是远端站点本身
