...
- **搜索与筛选：** 放大镜展开标题搜索框，Enter 搜索，Esc 收起。文章列表可按来源、未读、收藏等条件筛选；来源名完整显示，并以弱化数字显示该订阅全部未读数（含 0，不受搜索或归档筛选影响）；多页时分页按钮保持可触达。
...
- **继续阅读：** 本次运行内返回列表会恢复分页及文章所在位置，短暂标示刚打开的文章；重新打开文章会恢复阅读位置。重启应用或刷新 Web 页面后清除，不影响已读状态。
